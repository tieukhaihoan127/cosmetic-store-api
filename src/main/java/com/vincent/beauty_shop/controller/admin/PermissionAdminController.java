...
public class PermissionAdminController {
    ...
    @PostMapping
    public ResponseEntity<Permission> addPermission(@RequestBody PermissionCreateRequest request) {
        Permission permission = permissionService.createPermission(request);
        return new ResponseEntity<>(permission, HttpStatus.CREATED);
    }

    @PutMapping("/{id}")
    public ResponseEntity<Permission> updatePermission(@PathVariable Long id, @RequestBody PermissionUpdateRequest request) {
